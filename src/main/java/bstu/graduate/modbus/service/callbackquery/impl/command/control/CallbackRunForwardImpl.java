...
public class CallbackRunForwardImpl implements Callback {
    ...
    public List<BotApiMethod<?>> apiMethodProcessor(String callbackQueryData, Message message) {
        ...
        actions.add(runForward(messageId, chatId));
    ...
    public CallbackEnum getActionCallback() {
        return CallbackEnum.RUN_FORWARD;
    }

    private EditMessageText runForward(int messageId, long chatId) {
        deltaControl.runForward();

        return EditMessageText.builder()
                .messageId(messageId)
                .chatId(chatId)
                .text("Двигатель запущен вперед.")
