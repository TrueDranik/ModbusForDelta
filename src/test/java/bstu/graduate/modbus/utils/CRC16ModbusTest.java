...
class CRC16ModbusTest {
    ...
    void testSendBuffer() {
        ...
        byte[] firstSendBuffer = CRC16Modbus.getSendBuffer(CommandForDelta.RUN_FORWARD.getBytes());
        assertArrayEquals(firstResult, firstSendBuffer);

        byte[] secondResult = {1, 5, 0, 0, -1, 0, -116, 58};
        byte[] secondSendBuffer = CRC16Modbus.getSendBuffer(CommandForDelta.TURN_ON.getBytes());
    ...
    void testBufferHexString() {
        ...
        String result = CommandForDelta.RUN_FORWARD.getBytes();
